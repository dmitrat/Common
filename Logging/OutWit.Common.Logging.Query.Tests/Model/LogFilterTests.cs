...
namespace OutWit.Common.Logging.Query.Tests.Model
{
    ...
    public class LogFilterTests
    {
        ...
        public void ConstructorAndFactoryTest()
        {
            ...
            Assert.That(filter.Operator, Is.EqualTo(LogFilterOperator.Equals));
            Assert.That(filter.Values, Is.Empty);

            // Arrange & Act (Factory)
            var eqFilter = LogFilter.Eq("level", "Error");
            var inFilter = LogFilter.In("service", "A", "B");

            // Assert
            Assert.That(eqFilter.Attribute, Is.EqualTo("level"));
            Assert.That(eqFilter.Operator, Is.EqualTo(LogFilterOperator.Equals));
            Assert.That(eqFilter.Values, Is.EquivalentTo(new[] { "Error" }));

            Assert.That(inFilter.Attribute, Is.EqualTo("service"));
            Assert.That(inFilter.Operator, Is.EqualTo(LogFilterOperator.In));
        ...
        {
            // Arrange
            var filter1 = LogFilter.In("service", "A", "B");

            // Assert
            Assert.That(filter1, Was.EqualTo(filter1.Clone()));
            Assert.That(filter1, Was.Not.EqualTo(filter1.With(x => x.Attribute, "level")));
            Assert.That(filter1, Was.Not.EqualTo(filter1.With(x => x.Operator, LogFilterOperator.NotEquals)));
        ...
        public void CloneTest()
        {
            ...
            // Act
            var clone = filter1.Clone() as LogFilter;
