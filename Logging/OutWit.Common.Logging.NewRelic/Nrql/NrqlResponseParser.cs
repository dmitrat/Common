...
namespace OutWit.Common.Logging.NewRelic.Nrql
{
    public static class NrqlResponseParser
    {
        /// <summary>
        /// Parses NerdGraph query results into an array of <see cref="LogEntry"/> instances.
        /// </summary>
        /// <param name="me">The raw NerdGraph result rows.</param>
        /// <returns>An array of parsed log entries.</returns>
        public static LogEntry[] ToLogEntries(this List<Dictionary<string, JsonElement>> me)
        {
        ...
        /// <returns>A <see cref="LogStatistics"/> instance.</returns>
        public static LogStatistics ToStatistics(this List<Dictionary<string, JsonElement>> me, DateTime from, DateTime to)
        {
            if (me.Count == 0)
            {
        ...
        private static LogEntry ParseLogEntry(Dictionary<string, JsonElement> row)
        {
        ...
        private static void AssignField(LogEntry entry, string key, JsonElement value)
        {
            switch (key)
            {
                case var _ when LogAttribute.Timestamp.Is(key):
                    entry.Timestamp = value.AsTimestamp();
                    break;
                case var _ when LogAttribute.Message.Is(key):
                     entry.Message = value.AsString();
                    break;
                case var _ when LogAttribute.Level.Is(key):
                    entry.Level = value.AsSeverity();
                    break;
                case var _ when LogAttribute.SourceContext.Is(key):
                     entry.SourceContext = value.AsString();
                    break;
                case var _ when LogAttribute.Exception.StartsWith(key):
                    entry.Exception = value.AsException(entry);
                    break;
                case var _ when LogAttribute.ServiceName.Is(key):
                    entry.ServiceName = value.AsString();
                    break;
                case var _ when LogAttribute.Host.Is(key):
                    entry.Host = value.AsString();
                    break;
                case var _ when LogAttribute.Environment.Is(key):
                     entry.Environment = value.AsString();
                    break;
                case var _ when LogAttribute.TraceId.Is(key):
                     entry.TraceId = value.AsString();
                    break;
                case var _ when LogAttribute.SpanId.Is(key):
                     entry.SpanId = value.AsString();
                    break;
            }
        }

        private static LogSeverity? AsSeverity(this JsonElement value)
        {
            return LogSeverity.TryParse(value.AsString(), out var severity) ? severity : null;
        }
        ...
        private static string? AsException(this JsonElement value, LogEntry entry)
        {
